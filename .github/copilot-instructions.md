...
当用户请求涉及性能测试、压力测试、基准测试、吞吐量评估时，遵循以下规范。

- **运行环境**：**必须**在高性能机器（多核、大内存、高主频）上以 Release 模式运行，以获取有代表性的峰值数据
- **测试维度**：如有需要，并发维度应包含单线程和多线程（多线程应包含与当前 CPU 核心数相同的并发数）；操作维度应包含单一操作和批量操作
- **测试报告**：存放 `Doc/Benchmark/{测试主题}性能测试.md`（UTF-8），结构顺序如下：
  1. **测试结论**（放最前，先用一句话点明被测功能的用途，再用简单易懂的语言总结核心发现）
  2. 测试环境 → 测试方法 → 测试结果（BDN 原始表格，保留 Mean/Error/StdDev/Allocated）
  3. 核心指标（换算 msg/s、QPS 等业务指标）
  4. **对比分析**（纵向：同场景不同并发趋势，找出最优并发点；横向：不同方案同并发差异百分比）
