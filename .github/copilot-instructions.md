...
领域专用规则放在 `.github/instructions/` 下，满足触发信号时用 `file_search` 检查对应文件是否存在，存在则读取遵循。**未触发时不引入。**

| 触发信号 | 指令文件 |
|---------|---------|
| XCode/Cube/魔方/实体生成/Model.xml/数据库 CRUD/`NewLife.XCode` 引用/`*.xcode.xml`/项目名含 `.Data`/`XCode.*` 命名空间/**用户提及修改任意 `.xml` 文件** | `xcode.instructions.md` |
...
检索（**优先复用**现有实现）→ 评估（公共 API/兼容性/性能）→ 方案 → 实施 → 验证 → 说明
