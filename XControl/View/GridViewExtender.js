;(function () {
    ...
    tagArray.prototype.push = function () {
        ...
        for (i = 0; i < arguments.length; i++) {
            if (this.has(arguments[i])) continue; // 已有的不添加
            if (args.length > 0) {
                try {
                    for (j = 0; j < args.length; j++) {
                        if (this.__equal(arguments[i], args[j])) throw ''; // push参数中重复的不添加
                    }
    ...
    tagArray.prototype.has = function (obj, start) {
        ...
        for (i = start; i < this.length; i++) {
            if (this.__equal(obj, this[i])) return [i, this[i]];
        }
        return false;
    };
    ...
    function jsArray(arylike, s, n) {
        var ary, i;
        if (Object.prototype.toString.call(arylike) === '[object Array]') return arylike;
        if (typeof arylike.length === 'number') {
            if (typeof s === 'undefined') s = 0;
            if (s < 0) s = arylike.length - s;
            if (typeof n === 'undefined') n = arylike.length;
            if (n < 0) n = arylike.length - n;
            try {
    ...
    function filter(ary, func) {
        ...
        n = ary.length;
        for (i = 0; i < n; i++) {
            if (func(ary[i], i)) ary.push(ary[i]);
        }
        ary.splice(0, n);
        return ary;
    }
    ...
    w.GridViewExtender = {
        ...
        HighlightRow: function (ele, name, toggle) {
            var color, map, row, d, c, cc, gv = GridViewExtender.IsInExtendDataRow(ele);
            if (gv && gv[0] && gv[1]) {
                row = gv[1];
                gv = gv[0];
        ...
        ClickElement: function (tag, func) {
            return function (e, row, i) {
                var e, eles = filter(row.getElementsByTagName(tag), func);
                e = eles[0];
                if (e) {
                    fireClickEvent(e);
                }
