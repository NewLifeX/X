...
Dialog.prototype.show = function() {
    ...
    arr.push("      <div style=\"position: relative;cursor:pointer; float:right; margin:5px 0 0; _margin:4px 0 0;height:17px; width:28px; background-image:url(<%=WebResource("XControl.Box.Dialog.dialog_closebtn.gif")%>)\" onMouseOver=\"this.style.backgroundImage='url(<%=WebResource("XControl.Box.Dialog.dialog_closebtn_over.gif") %>)'\" onMouseOut=\"this.style.backgroundImage='url(<%=WebResource("XControl.Box.Dialog.dialog_closebtn.gif")%>)'\" drag='false' onClick=\"Dialog.getInstance('" + this.ID + "').CancelButton.onclick.apply(Dialog.getInstance('" + this.ID + "').CancelButton,[]);\"></div></td>");
