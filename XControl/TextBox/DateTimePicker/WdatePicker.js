...
(function () {
    ...
    A = L(); // 不能注释 后续还有地方会用到A
    if (_.$wdate) M('<%= WebResource("XControl.TextBox.DateTimePicker.skin.WdatePicker.css") %>');
    V = X;
    if (_.$crossFrame) {
        try {
    ...
    function K(H, $) {
        ...
        if (!$dp.dd || $dp.eCont || ($dp.lang && $dp.realLang && $dp.realLang.name != $dp.lang && $dp.getLangIndex && $dp.getLangIndex($dp.lang) >= 0)) {
            ...
            if (_.$dpPath == "") F(A);
            var B = "<iframe style=\"width:1px;height:1px\" src=\"" + '<%= WebResource("XControl.TextBox.DateTimePicker.My97DatePicker.htm") %>' + "\" frameborder=\"0\" border=\"0\" scrolling=\"no\"></iframe>";
            if ($dp.eCont) {
