...
var $dp,WdatePicker;(function(){
    ...
    var skins = {};
    skins['default'] = '<%= WebResource("XControl.TextBox.DateTimePicker.skin.default.datepicker.css") %>';
    skins['blue'] = '<%= WebResource("XControl.TextBox.DateTimePicker.skin.blue.datepicker.css") %>';
    skins['green'] = '<%= WebResource("XControl.TextBox.DateTimePicker.skin.green.datepicker.css") %>';
    skins['red'] = '<%= WebResource("XControl.TextBox.DateTimePicker.skin.red.datepicker.css") %>';

    var langs = {};
    skins['en'] = '<%= WebResource("XControl.TextBox.DateTimePicker.lang.en.js") %>';
    skins['zh-cn'] = '<%= WebResource("XControl.TextBox.DateTimePicker.lang.zh-cn.js") %>';
    skins['zh-tw'] = '<%= WebResource("XControl.TextBox.DateTimePicker.lang.zh-tw.js") %>';
    ...
    switch(navigator.appName){
        ...
        case"Opera":b=true;break;
        default:I=true;break
    }
    A=GetPath();
    if(_.$wdate)LoadCss('<%=WebResource("XControl.TextBox.DateTimePicker.skin.WdatePicker.css") %>');
    V=X;
    if(_.$crossFrame){
        try{
    ...
    function G($){
        ...
        while($!=V){
            ...
            for(var F=0;F<D.length;F++){try{if(D[F].contentWindow==$){var E=W(D[F]);A+=E.left;_+=E.top;break}}catch(B){}}
            $=$.parent
        }
    ...
    function U(E,_){
        ...
        if(_){
            ...
            if($dp.status==0){U(null,true);return}
            if($dp.status!=2)return;
            var C=A();
            if(C){$dp.srcEl=C.srcElement||C.target;C.cancelBubble=true}
    ...
    function K(H,$){
        ...
        if(!$dp.dd||$dp.eCont||($dp.lang&&$dp.realLang&&$dp.realLang.name!=$dp.lang&&$dp.getLangIndex&&$dp.getLangIndex($dp.lang)>=0)){
            ...
            if(_.$dpPath=="")F(A);
            var B="<iframe src='<%= WebResource("XControl.TextBox.DateTimePicker.My97DatePicker.htm") %>' frameborder=\"0\" border=\"0\" scrolling=\"no\"></iframe>";
            if($dp.eCont){$dp.eCont.innerHTML=B;Z($dp.eCont.childNodes[0],Y)}
                ...
                $dp.dd.innerHTML=B;V[O].body.insertBefore($dp.dd,V[O].body.firstChild);
        ...
        function C(){
            ...
            else F+=A.left+$.leftM;$dp.dd.style.top=B+"px";
