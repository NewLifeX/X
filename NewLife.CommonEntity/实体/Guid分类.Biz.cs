...
namespace NewLife.CommonEntity
{
    ...
    public class GuidCategory : GuidCategory<GuidCategory> { }

    /// <summary>Guid分类</summary>
    public partial class GuidCategory<TEntity> : EntityTree<TEntity> where TEntity : GuidCategory<TEntity>, new()
    {
        #region 扩展属性﻿
        #endregion

        #region 扩展查询﻿
        /// <summary>根据主键查询一个Guid分类实体对象用于表单编辑</summary>>
        /// <param name="guid">编号</param>
        /// <returns></returns>
        [DataObjectMethod(DataObjectMethodType.Select, false)]
        public static TEntity FindByKeyForEdit(Guid guid)
        {
            TEntity entity = Find(new String[] { _.Guid }, new Object[] { guid });
            if (entity == null)
            {
                entity = new TEntity();
            }
        ...
        /// <param name="parentguid">父分类</param>
        /// <returns></returns>
        [DataObjectMethod(DataObjectMethodType.Select, false)]
        public static EntityList<TEntity> FindAllByParentGuid(String parentguid)
        {
            if (Meta.Count >= 1000)
        ...
        /// <returns></returns>
        [DataObjectMethod(DataObjectMethodType.Select, false)]
        public static TEntity FindByGuid(Guid guid)
        {
            if (Meta.Count >= 1000)
