...
public class EventHubIntegrationTests
{
    ...
    public async Task MultiClient_Subscribe_Publish_SenderExcluded()
    {
        ...
        c1Ctx["Handler"] = h1;
        await hub.HandleAsync("event#chat#c1#subscribe", c1Ctx);

        // c2 订阅
        var h2 = new TestEventHandler();
        var c2Ctx = new EventContext();
        c2Ctx["Handler"] = h2;
        await hub.HandleAsync("event#chat#c2#subscribe", c2Ctx);

        // sender 也订阅（发布时不应收到自己的消息）
        var hSender = new TestEventHandler();
        var senderSubCtx = new EventContext();
        senderSubCtx["Handler"] = hSender;
        await hub.HandleAsync("event#chat#sender#subscribe", senderSubCtx);

        // 发布时传入上下文以触发发送方排除逻辑
        var pubCtx = new EventContext();
        var rs = await hub.HandleAsync("event#chat#sender#{\"Message\":\"hello\"}", pubCtx);
    ...
    public async Task PacketFlow_SubscribeThenPublish_EndToEnd()
    {
        ...
        await hub.HandleAsync(new ArrayPacket("event#device#c1#subscribe".GetBytes()), subCtx);

        // 通过二进制报文发布
        var pubPacket = new ArrayPacket("event#device#sender#{\"Message\":\"ping\"}".GetBytes());
        var rs = await hub.HandleAsync(pubPacket, null);
    ...
    public async Task Unsubscribe_AutoCleansUpBus_ThenPublishReturnsZero()
    {
        ...
        var subCtx = new EventContext();
        subCtx["Handler"] = handler;
        await hub.HandleAsync("event#device#c1#subscribe", subCtx);
        Assert.True(hub.TryGetBus<TestEvent>("device", out _));

        // 取消订阅 — 总线应被清理
        await hub.HandleAsync("event#device#c1#unsubscribe", null);
        Assert.False(hub.TryGetBus<TestEvent>("device", out _));

        // 此时发布应返回 0
        var rs = await hub.HandleAsync("event#device#sender#{\"Message\":\"lost\"}", null);
        Assert.Equal(0, rs);
    }
    ...
    public async Task Topics_DoNotInterfere()
    {
        ...
        hub.Add("topicA", handlerA);
        hub.Add("topicB", handlerB);

        await hub.HandleAsync("event#topicA#sender#{\"Message\":\"msgA\"}", null);
