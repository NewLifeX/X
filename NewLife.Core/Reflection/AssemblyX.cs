...
public class AssemblyX
{
    ...
    public String Location
    {
        get
        {
            ...
            {
#if !__CORE__
                return Asm == null || Asm is _AssemblyBuilder ? null : Asm.Location;
#else
    ...
    private static Assembly OnReflectionOnlyAssemblyResolve(Object sender, ResolveEventArgs args)
    {
        ...
        if (flag) XTrace.WriteLine("请求只反射加载[{0}]", args.Name);
        //if (!flag) return null;
    ...
    private static Assembly OnAssemblyResolve(Object sender, ResolveEventArgs args)
    {
        ...
        if (flag) XTrace.WriteLine("请求加载[{0}]", args.Name);
        //if (!flag) return null;
        ...
        if (d <= y && d >= y - 10)
        {
            ...
            else
            {
                ...
                if (p > 0)
                {
                    ...
                    if (str.Length >= 4 + 1 + 4)
                    {
                        s = str.Substring(str.Length - 4).ToInt();
