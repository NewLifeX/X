...
namespace System
{
	internal abstract class Lightup
	{
		...
		{
			Type[] argumentTypes = Enumerable.ToArray<Type>(Enumerable.Select<object, Type>(parameters, (object p) => p.GetType()));
			Func<object[], T> func = Lightup.CreateActivator<T>(argumentTypes);
		...
		{
			if (typeof(T).GetConstructor(argumentTypes) == null)
			{
				return null;
			}
