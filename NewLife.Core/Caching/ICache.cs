...
/// <para>通用语义约定：</para>
/// <list type="bullet">
/// <item><description>过期时间单位为秒（除非另有 TimeSpan 参数）。<c>expire &lt; 0</c> 表示采用 <see cref="Expire"/> 默认值，<c>expire == 0</c> 表示永不过期，<c>expire &gt; 0</c> 表示相对过期（从现在起）。</description></item>
/// <item><description>已过期的键应视为不存在；实现可在访问时惰性清理。</description></item>
/// <item><description>远端缓存（如 Redis）上 <see cref="Keys"/>、<see cref="Count"/>、<see cref="Search(string, int, int)"/> 可能是高耗时/高复杂度操作，仅用于诊断或小规模数据。</description></item>
/// </list>
/// </remarks>
public interface ICache
{
    ...
    /// <remarks>
    /// 作为 <c>expire &lt; 0</c> 时的回退值；实现应使用“相对过期”（TTL，从现在起计时）。
    /// </remarks>
    ...
    /// <param name="expire">过期时间，秒。小于0时采用默认缓存时间<seealso cref="Expire"/>；0 表示永不过期；大于0表示从现在起相对过期</param>
    ...
    /// <param name="expire">过期时间，秒。小于0时采用默认缓存时间<seealso cref="Expire"/>；0 表示永不过期</param>
    ...
    /// <param name="expire">过期时间，秒。小于0时采用默认缓存时间<seealso cref="Cache.Expire"/>；0 表示永不过期</param>
    /// <returns>是否成功添加。若键已存在返回 <c>false</c></returns>
    Boolean Add<T>(String key, T value, Int32 expire = -1);

    /// <summary>设置新值并获取旧值，原子操作</summary>
    /// <remarks>
    /// 常常配合Increment使用，用于累加到一定数后重置归零，又避免多线程冲突。若键不存在，返回默认值并设置新值。
    /// </remarks>
    /// <typeparam name="T">值类型</typeparam>
    /// <param name="key">键</param>
    /// <param name="value">值</param>
    ...
    /// <remarks>
    /// 纯内存实现通常可返回 <c>0</c> 或直接忽略；持久化实现可用于批量提交。
    /// </remarks>
    Int32 Commit();
