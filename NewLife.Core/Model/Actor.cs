...
public abstract class Actor : DisposeBase, IActor
{
    ...
    protected virtual Task ReceiveAsync(ActorContext context, CancellationToken cancellationToken) => TaskEx.FromResult(0);

    /// <summary>批量处理消息。批大小大于1时使用该方法</summary>
    /// <param name="contexts">上下文集合</param>
    /// <param name="cancellationToken">取消通知</param>
    protected virtual Task ReceiveAsync(ActorContext[] contexts, CancellationToken cancellationToken) => TaskEx.FromResult(0);
#elif NET45
    /// <summary>处理消息。批大小为1时使用该方法</summary>
    /// <param name="context">上下文</param>
    /// <param name="cancellationToken">取消通知</param>
