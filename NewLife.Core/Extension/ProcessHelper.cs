...
public static class ProcessHelper
{
    ...
    public static String[]? GetCommandLineArgs(Int32 processId)
    {
        ...
        else if (Runtime.Windows)
        {
            ...
            try
            {
                ...
                var rs = ReadProcessMemory(processHandle, new IntPtr(pbi.PebBaseAddress.ToInt64() + 0x20), buffer, (UInt32)buffer.Length, out _);
                if (!rs) return null;

                var paramAddress = (IntPtr)BitConverter.ToInt64(buffer, 0);
                buffer = new Byte[Marshal.SizeOf(typeof(UNICODE_STRING))];
                rs = ReadProcessMemory(processHandle, new IntPtr(paramAddress.ToInt64() + 0x70), buffer, (UInt32)buffer.Length, out _);
                if (!rs) return null;
    ...
    public static Int32 Run(this String cmd, String? arguments = null, Int32 msWait = 0, Action<String?>? output = null, Action<Process>? onExit = null, String? working = null)
    {
        ...
        if (!String.IsNullOrEmpty(working)) si.WorkingDirectory = working;
        // 对于控制台项目，这里需要捕获输出
        if (msWait > 0)
        {
