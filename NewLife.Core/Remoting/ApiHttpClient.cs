...
public class ApiHttpClient : DisposeBase, IApiClient, IConfigMapping, ILogFeature
{
    ...
    private void Init(String urls)
    {
        if (!urls.IsNullOrEmpty() && urls != _lastUrls)
        {
            ...
            var ss = urls.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
    ...
    /// <param name="args">参数</param>
    /// <returns></returns>
    public TResult Get<TResult>(String action, Object args = null) => Task.Run(() => GetAsync<TResult>(action, args)).Result.Result;
    ...
    /// <param name="args">参数</param>
    /// <returns></returns>
    public TResult Post<TResult>(String action, Object args = null) => Task.Run(() => PostAsync<TResult>(action, args)).Result.Result;
