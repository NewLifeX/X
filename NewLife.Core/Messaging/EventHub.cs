...
/// </remarks>
public class EventHub<TEvent> : IEventDispatcher<IPacket>, IEventDispatcher<String>
{
    #region 属性
    private readonly ConcurrentDictionary<String, DispatchCallback<TEvent>> _eventBuses = new();
    #endregion
    ...
    public void Add(String topic, IEventDispatcher<TEvent> dispatcher)
    {
        ...
        _eventBuses[topic] = dispatcher.DispatchAsync;
    }
    ...
    /// <returns></returns>
    public Boolean TryGetValue(String topic, [MaybeNullWhen(false)] out DispatchCallback<TEvent> action) => _eventBuses.TryGetValue(topic, out action);
    ...
    {
        if (_eventBuses.TryGetValue(topic, out var action))
        {
    ...
    public virtual Task<Int32> DispatchAsync(String topic, String clientId, TEvent @event, CancellationToken cancellationToken = default)
    {
        if (topic.IsNullOrEmpty()) throw new ArgumentNullException(nameof(topic));

        if (!_eventBuses.TryGetValue(topic, out var action)) return Task.FromResult(0);

        return action(@event, cancellationToken);
    }
    #endregion
}
