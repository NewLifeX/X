...
namespace NewLife.YWS.Entities
{
    ...
    public partial class Customer : YWSEntityBase<Customer>
    {
        ...
        [NonSerialized]
		private EntityList<Feedliquor> _Feedliquors;
		/// <summary>该客户所拥有的液料规格集合</summary>
		[XmlIgnore]
		public EntityList<Feedliquor> Feedliquors
		{
			get
			{
				if (_Feedliquors == null && ID > 0 && !Dirtys.ContainsKey("Feedliquors"))
                {
					_Feedliquors = Feedliquor.FindAllByCustomerID(ID);
					Dirtys["Feedliquors"] = true;
				}
				return _Feedliquors;
			}
			set { _Feedliquors = value; }
		}

        [NonSerialized]
        private CustomerType _CustomerType;
        /// <summary>该客户所对应的客户类型</summary>
        [XmlIgnore]
        public CustomerType CustomerType
        {
            get
            {
                if (_CustomerType == null && CustomerTypeID > 0 && !Dirtys.ContainsKey("CustomerType"))
                {
                    _CustomerType = CustomerType.FindByID(CustomerTypeID);
                    Dirtys["CustomerType"] = true;
                }
        ...
        [NonSerialized]
		private EntityList<Record> _Records;
		/// <summary>该客户所拥有的交易记录集合</summary>
		[XmlIgnore]
		public EntityList<Record> Records
		{
			get
			{
				if (_Records == null && ID > 0 && !Dirtys.ContainsKey("Records"))
                {
					_Records = Record.FindAllByCustomerID(ID);
					Dirtys["Records"] = true;
				}
				return _Records;
			}
		...
		/// <summary>该客户所拥有的维修保养记录集合</summary>
		[XmlIgnore]
		public EntityList<Maintenance> Maintenances
		{
			get
			{
                ...
                {
					_Maintenances = Maintenance.FindAllByCustomerID(ID);
        ...
        [NonSerialized]
		private EntityList<Machine> _Machines;
		/// <summary>该客户所拥有的机器零件规格集合</summary>
		[XmlIgnore]
		public EntityList<Machine> Machines
		{
			get
			{
				if (_Machines == null && ID > 0 && !Dirtys.ContainsKey("Machines"))
                {
					_Machines = Machine.FindAllByCustomerID(ID);
					Dirtys["Machines"] = true;
				}
				return _Machines;
			}
			set { _Machines = value; }
        ...
        public static Customer FindByKeyForEdit(Int32 id)
        {
            Customer entity = Find(new String[] { _.ID }, new Object[] { id });
            if (entity == null)
            {
                entity = new Customer();
            }
        ...
        /// <returns></returns>
        [DataObjectMethod(DataObjectMethodType.Select, false)]
        public static Customer FindByNo(String no)
        {
            if (Meta.Count >= 1000)
                return Find(new String[] { _.No }, new Object[] { no });
            else // 实体缓存
                return Meta.Cache.Entities.Find(_.No, no);
            // 单对象缓存
        ...
        /// <returns></returns>
        [DataObjectMethod(DataObjectMethodType.Select, false)]
        public static EntityList<Customer> FindAllByCustomerTypeID(Int32 customertypeid)
        {
            if (Meta.Count >= 1000)
                return FindAll(new String[] { _.CustomerTypeID }, new Object[] { customertypeid });
            else // 实体缓存
                return Meta.Cache.Entities.FindAll(_.CustomerTypeID, customertypeid);
        }
        #endregion
        ...
        //{
        //    if (Feedliquors != null) Feedliquors.Delete();
        //    if (Maintenances != null) Maintenances.Delete();
        //    if (Machines != null) Machines.Delete();
        //    if (Records != null) Records.Delete();
        ...
        public static EntityList<Customer> Search(String name, String depart, String linkman, String orderClause, Int32 startRowIndex, Int32 maximumRows)
        {
        ...
        /// <param name="maximumRows">最大返回行数</param>
        /// <returns>记录数</returns>
        public static Int32 SearchCount(String name, String depart, String linkman, String orderClause, Int32 startRowIndex, Int32 maximumRows)
        {
        ...
        private static String SearchWhere(String name, String depart, String linkman)
        {
            ...
            if (!String.IsNullOrEmpty(depart)) sb.AppendFormat(" And {0} like '%{1}%'", _.Department, depart.Replace("'", "''"));
            if (!String.IsNullOrEmpty(linkman)) sb.AppendFormat(" And {0} like '%{1}%'", _.Linkman, linkman.Replace("'", "''"));
