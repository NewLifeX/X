    ...
    public class UserTests
    {
        ...
        public void TestRoleIds()
        {
            var user = new User
            {
                ...
                RoleIds = new[] { 3, 2, 1, 7, 4 },
            };
            user.Insert();

            Assert.Equal(1, user.RoleID);
            Assert.Equal(4, user.RoleIds.Length);
            Assert.Equal(new[] { 2, 3, 4, 7 }, user.RoleIds);

            var user2 = User.FindByKey(user.ID);
            Assert.Equal(1, user2.RoleID);
            Assert.Equal(4, user2.RoleIds.Length);
            Assert.Equal(new[] { 2, 3, 4, 7 }, user2.RoleIds);

            user2.RoleIds = new[] { 5, 3, 9, 2 };
            user2.Update();

            var user3 = User.FindByKey(user.ID);
            Assert.Equal(1, user3.RoleID);
            Assert.Equal(4, user3.RoleIds.Length);
            Assert.Equal(new[] { 2, 3, 5, 9 }, user3.RoleIds);
