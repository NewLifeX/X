...
#pragma warning disable 618
            /// <summary>是否是在线翻译服务的来源</summary>
            /// <param name="s"></param>
            /// <returns></returns>
            [Obsolete("建议用新的方法系列,Is Get TryGetKnowType方法")]
            public static bool IsTranslateWS(string s)
            {
                return s.StartsWith(TranslateWS, StringComparison.OrdinalIgnoreCase);
            }
            /// <summary>返回指定在线翻译服务名称的来源</summary>
            /// <param name="s"></param>
            /// <returns></returns>
            [Obsolete("建议用新的方法系列,Is Get TryGetKnowType方法")]
            public static string GetTranslateWS(string s)
            {
                if (IsTranslateWS(s))
                {
                    return TranslateWS + s.Substring(TranslateWS.Length);
                }
                else
                {
                    return TranslateWS + " " + s;
                }
            }
            /// <summary>是否是翻译条目添加接口的来源</summary>
            /// <param name="s"></param>
            /// <returns></returns>
            [Obsolete("建议用新的方法系列,Is Get TryGetKnowType方法")]
            public static bool IsTranslateNew(string s)
            {
                return s.StartsWith(TranslateNew, StringComparison.OrdinalIgnoreCase);
            }
            /// <summary>返回指定名称的翻译条目添加接口的来源</summary>
            /// <param name="s"></param>
            /// <returns></returns>
            [Obsolete("建议用新的方法系列,Is Get TryGetKnowType方法")]
            public static string GetTranslateNew(string s)
            {
                if (IsTranslateNew(s))
                {
                    return TranslateNew + s.Substring(TranslateNew.Length);
                }
                else
                {
                    return TranslateNew + " " + s;
                }
            }
            /// <summary>指定字符串是否是已知类型</summary>
            /// <param name="s"></param>
            /// <param name="type"></param>
            /// <returns></returns>
            [Obsolete("建议用新的方法系列,Is Get TryGetKnowType方法")]
            public static bool IsKnowType(string s, out string type)
            {
                foreach (string t in KnowTypes)
                {
                ...
                if (IsTranslateWS(s))
                {
                    type = GetTranslateWS(s);
                    return true;
                }
                if (IsTranslateNew(s))
                {
                    type = GetTranslateNew(s);
                    return true;
                }
                type = null;
                return false;
