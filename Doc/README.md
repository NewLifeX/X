...
| **消息队列事件总线** | [消息队列事件总线QueueEventBus.md](消息队列事件总线QueueEventBus.md) | 分布式事件总线 |
| **令牌HTTP过滤器** | [令牌HTTP过滤器TokenHttpFilter.md](令牌HTTP过滤器TokenHttpFilter.md) | HTTP令牌自动注入 |
| **数据模型接口** | [数据模型接口IExtend_IModel_IFilter.md](数据模型接口IExtend_IModel_IFilter.md) | 扩展属性/模型索引/过滤器 |
...
| **数据包编码器** | [数据包编码器IPacketEncoder.md](数据包编码器IPacketEncoder.md) | 对象与数据包互转 |
| **数据集** | [数据集DbTable.md](数据集DbTable.md) | 内存数据表 |
| **雪花算法** | [雪花算法Snowflake.md](雪花算法Snowflake.md) | 分布式唯一ID |
| **降采样与插值** | [降采样与插值算法框架.md](降采样与插值算法框架.md) | 时序数据降采样与插值 |
